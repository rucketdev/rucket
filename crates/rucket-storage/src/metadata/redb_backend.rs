...
use rucket_core::types::{
    BucketInfo, BucketLoggingStatus, CorsConfiguration, CorsRule, ETag, MultipartUpload,
    ObjectMetadata, Part, StorageClass, Tag, TagSet, VersioningStatus, WebsiteConfiguration,
};
...
/// Logging configuration table: bucket_name -> BucketLoggingStatus (bincode)
const BUCKET_LOGGING: TableDefinition<'_, &str, &[u8]> = TableDefinition::new("bucket_logging");
...
impl RedbMetadataStore {
    ...
    pub fn open(path: &Path, sync_strategy: SyncStrategy) -> Result<Self> {
        ...
        // This ensures tables exist before any read operations
        {
            ...
            let _ = txn.open_table(BUCKET_LOGGING).map_err(db_err)?;
...
impl MetadataBackend for RedbMetadataStore {
    ...
    async fn get_bucket_logging(&self, name: &str) -> Result<Option<BucketLoggingStatus>> {
        ...
        tokio::task::spawn_blocking(move || {
            let read_txn = db.begin_read().map_err(db_err)?;
            let table = read_txn.open_table(BUCKET_LOGGING).map_err(db_err)?;

            match table.get(bucket_name.as_str()).map_err(db_err)? {
                Some(value) => {
                    let config: BucketLoggingStatus =
                        bincode::deserialize(value.value()).map_err(|e| {
                            Error::Database(format!("Failed to deserialize logging config: {e}"))
                        })?;
                    Ok(Some(config))
                }
                None => Ok(None),
    ...
    async fn put_bucket_logging(&self, name: &str, config: BucketLoggingStatus) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            {
                let mut table = txn.open_table(BUCKET_LOGGING).map_err(db_err)?;
                let bytes = bincode::serialize(&config).map_err(|e| {
                    Error::Database(format!("Failed to serialize logging config: {e}"))
                })?;
                table.insert(bucket_name.as_str(), bytes.as_slice()).map_err(db_err)?;
            }
